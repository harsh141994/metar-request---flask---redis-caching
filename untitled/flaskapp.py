...
@app.route('/metar/info/<code>/<int:nocache>')
def metar(code, nocache):
    obj = SimpleCache()
    if nocache==1:
        print "new call.."
        r = requests.get('http://tgftp.nws.noaa.gov/data/observations/metar/stations/'+code+'.TXT');
        s = r.text
        lis = s.splitlines()
        metar_str = str(lis[1])
        ...
        js = json.dumps(lis)
        obj.store_json(code, js, 5)

    else:
        try:
            obj.get_json(code)
        except:
            print "key not created or key has expired..."
            r = requests.get('http://tgftp.nws.noaa.gov/data/observations/metar/stations/' + code + '.TXT');
            s = r.text
            lis = s.splitlines()
            metar_str = str(lis[1])
